...
def _write_runtime_npz(path: Path) -> None:
    ...
    env_channels = np.zeros((frames, h, w, 8), dtype=np.float32)
    ...
    env_channels[..., 7] = (xx + yy) / max(h + w - 2, 1)
    world_logvar = np.zeros((frames, h, w, 8), dtype=np.float32)
    ...
    np.savez_compressed(
        ...
        world_belief=np.zeros((frames, h, w, 8), dtype=np.float32),
...
def test_export_openframeworks_frames_writes_manifest_and_pngs(tmp_path: Path) -> None:
    ...
    for key in ("life", "field", "body", "aura"):
